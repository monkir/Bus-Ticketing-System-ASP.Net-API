...
export default function App() {
    ...
    const onSubmit = async data => {
        ...
        try {
            const response = await axios.post(
                process.env.NEXT_PUBLIC_api_root + '/api/user/account/deposit/' + ammount,
                content,
                {
            ...
            // setTimeout(() => { router.push('/employee/manage/employee') }, 2000);
        }
        catch (e) {
            try {
                console.log(e)
    ...
    return (
        <>
            ...
            <EmployeeHeader title="Bus Ticketing System" pagename="Bus provider: Deposit money"/>
                    ...
                    <div className="sm:mx-auto sm:w-full sm:max-w-sm">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className='bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full'>
                            ...
                            <div class="grid grid-flow-col justify-stretch space-x-2 mx-2">
                                <Link className='btn btn-info' href={'/employee/account'}>Cancel</Link>
                                <input
                                    type="submit"
                                    value="Edit"
            ...
            {/* Modal */}
            <dialog id="my_modal_1" className="modal" onClose={() => { router.push('/employee/account') }}>

                <div className="modal-box">
                    <h3 className="font-bold text-lg">Hello!</h3>
                    <p className="py-4">Amount is added successfully</p>
                    <div className="modal-action">
                        <button onClick={() => { router.push('/employee/account') }} className="btn">Ok</button>
                    </div>
                </div>
            </dialog>
            <EmployeeFooter />
