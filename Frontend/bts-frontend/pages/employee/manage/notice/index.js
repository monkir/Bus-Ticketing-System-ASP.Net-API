      ...
      <div className="overflow-x-auto px-10 min-h-[70vh]">
        <div className="grid justify-items-stretch">
          <div className=" flex justify-self-center w-1/2">
            <Link className="btn btn-active btn-outline w-1/4" href={'/employee/manage/notice/add'}>Add New Notice</Link>
            {/* Search Box */}
            <input
              type="text"
              name="search"
          ...
          <tbody>
            {data.map(item => (
              <tr key={item.id}>
                ...
                <td>{item.emp_id}</td>
                <td>
                  <Link className=" btn btn-info mx-1" href={"/employee/manage/notice/edit/" + item.id}>Edit</Link>
