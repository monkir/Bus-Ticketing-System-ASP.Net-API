...
export default function App() {
    ...
    return (
        <>
            <AdminHeader title="Bus Ticketing System" pagename="Employee: Manage Bus Provider"/>
                    ...
                    <div className="sm:mx-auto sm:w-full sm:max-w-sm">
                        <form onSubmit={handleSubmit(onSubmit)} className='bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full'>
                            ...
                            <div class="grid grid-flow-col justify-stretch space-x-2 mx-2">
                                <Link className='btn btn-info' href={'/admin/profile'}>Cancel</Link>
                                <input
                                    type="submit"
            ...
            {/* Modal */}
            <dialog id="my_modal_1" className="modal" onClose={() => { router.push('/admin/profile') }}>
                <div className="modal-box">
                    <h3 className="font-bold text-lg">Hello!</h3>
                    <p className="py-4">{message}</p>
                    <div className="modal-action">
                        <button onClick={() => { router.push('/admin/profile') }} className="btn">Ok</button>
                    </div>
                </div>
            </dialog>
            <AdminFooter />
