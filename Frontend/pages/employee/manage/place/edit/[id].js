...
export default function App() {
    ...
    async function fetchData() {
        try {
            const response = await axios.get(
                process.env.NEXT_PUBLIC_api_root + '/api/employee/place/get/' + id,
                {
    ...
    const onSubmit = async data => {
        ...
        try {
            const response = await axios.put(
                process.env.NEXT_PUBLIC_api_root + '/api/employee/place/update',
                content,
                {
                    headers: { 'Authorization': sessionStorage.getItem('token_string') }
                }
    ...
    return (
        <>
            ...
            <EmployeeHeader title="Bus Ticketing System" pagename="Employee: Edit Place"></EmployeeHeader>
                                    ...
                                    <form onSubmit={handleSubmit(onSubmit)} className='bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full'>
                                        <div className="form-control w-full max-w-xs">
                                            ...
                                            <input type="text" placeholder="name" defaultValue={data.name} className="input input-bordered w-full max-w-xs"
                                                {...register("name", { required: { value: true, message: "name name is required" } })} />
                                        ...
                                        <div class="grid grid-flow-col justify-stretch space-x-2 mx-2">
                                            <Link className='btn btn-info' href={'/employee/manage/place'}>Cancel</Link>
                                            <input
                                                type="submit"
                                                value="Edit"
            ...
            {/* Modal */}
            <dialog id="my_modal_1" className="modal" onClose={() => { router.push('/employee/manage/place') }}>

                <div className="modal-box">
                    <h3 className="font-bold text-lg">Hello!</h3>
                    <p className="py-4">Place is updated successfully</p>
                    <div className="modal-action">
                        <button onClick={() => { router.push('/employee/manage/place') }} className="btn">Ok</button>
                    </div>
                </div>
            </dialog>
            <EmployeeFooter />
