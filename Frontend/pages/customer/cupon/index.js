...
export default function Example() {
    ...
    try {
      if (searchValue != "") {
        const response = await axios.get(
          process.env.NEXT_PUBLIC_api_root + '/api/customer/cupon/search/' + searchValue,
          {
        ...
        const response = await axios.get(
          process.env.NEXT_PUBLIC_api_root + '/api/customer/cupon' + searchValue,
          {
            headers: { 'Authorization': sessionStorage.getItem('token_string') }
          }
        )
  ...
  return (
    <>
      <CustomerHeader title="Bus Ticketing System" pagename="Customer Panel: Cupon"></CustomerHeader>
